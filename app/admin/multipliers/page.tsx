...
export default function AdminMultipliers() {
  ...
  function handleCancelAge() {
    ...
    setEditingAge(null);
    setAgeFormData({ ageRange: "", ageRangeEn: "", multiplier: "1.00", displayOrder: 0 });
  }

  function handleCancelPurpose() {
    setShowPurposeForm(false);
    setEditingPurpose(null);
    setPurposeFormData({ purpose: "", purposeEn: "", multiplier: "1.00", displayOrder: 0 });
  }
  ...
  async function handleAgeSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      alert(editingAge ? "Multiplier updated successfully!" : "Multiplier created successfully!");
      handleCancelAge();
      fetchMultipliers();
    } catch (error) {
      alert(error instanceof Error ? error.message : "Failed to save multiplier");
    }
  }
  ...
  async function handlePurposeSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      alert(editingPurpose ? "Multiplier updated successfully!" : "Multiplier created successfully!");
      handleCancelPurpose();
      fetchMultipliers();
    } catch (error) {
      alert(error instanceof Error ? error.message : "Failed to save multiplier");
  ...
  async function handleDeleteAge(id: string) {
    ...
    try {
      const response = await fetch(`/api/admin/property-age-multipliers/${id}`, {
        method: "DELETE",
      });
      ...
      alert(error instanceof Error ? error.message : "Failed to delete multiplier");
    }
  }
  ...
  async function handleDeletePurpose(id: string) {
    ...
    try {
      const response = await fetch(`/api/admin/inspection-purpose-multipliers/${id}`, {
        method: "DELETE",
      });
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-primary">Multipliers Management</h1>
            <p className="text-sm text-gray-600 mt-1">Manage property age and inspection purpose multipliers</p>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList>
            ...
            <TabsTrigger value="inspection-purpose">Inspection Purpose</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="property-age">
            {showAgeForm && (
              <Card className="mb-6">
                <CardHeader>
                  <CardTitle>{editingAge ? "Edit Property Age Multiplier" : "Create New Property Age Multiplier"}</CardTitle>
                </CardHeader>
                <CardContent>
                  <form onSubmit={handleAgeSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => setAgeFormData({ ...ageFormData, ageRange: e.target.value })}
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => setAgeFormData({ ...ageFormData, ageRangeEn: e.target.value })}
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => setAgeFormData({ ...ageFormData, multiplier: e.target.value })}
                          placeholder="1.00"
                          required
                        />
                        <p className="text-xs text-gray-500">e.g., 0.95 = -5%, 1.15 = +15%</p>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => setAgeFormData({ ...ageFormData, displayOrder: parseInt(e.target.value) })}
                          required
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button type="button" variant="outline" onClick={handleCancelAge}>
                        Cancel
                      </Button>
                    </div>
                  </form>
              ...
              {propertyAges.map((age) => (
                <Card key={age.id}>
                  <CardHeader>
                    ...
                    {age.ageRangeEn && <CardDescription>{age.ageRangeEn}</CardDescription>}
                    <CardAction>
                      <div className="flex gap-2">
                        <Button
                          variant="outline"
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      ...
                      <div>
                        ...
                        <span className={age.isActive ? "text-green-600" : "text-red-600"}>
          ...
          <TabsContent value="inspection-purpose">
            {showPurposeForm && (
              <Card className="mb-6">
                <CardHeader>
                  <CardTitle>
                    {editingPurpose ? "Edit Inspection Purpose Multiplier" : "Create New Inspection Purpose Multiplier"}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <form onSubmit={handlePurposeSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => setPurposeFormData({ ...purposeFormData, purpose: e.target.value })}
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => setPurposeFormData({ ...purposeFormData, purposeEn: e.target.value })}
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => setPurposeFormData({ ...purposeFormData, multiplier: e.target.value })}
                          placeholder="1.00"
                          required
                        />
                        <p className="text-xs text-gray-500">e.g., 0.90 = -10%, 1.10 = +10%</p>
                      </div>

                      <div className="space-y-2">
                        <Label htmlFor="purpose-displayOrder">Display Order</Label>
                        <Input
                          id="purpose-displayOrder"
                          type="number"
                          value={purposeFormData.displayOrder}
                          onChange={(e) => setPurposeFormData({ ...purposeFormData, displayOrder: parseInt(e.target.value) })}
                          required
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                      <Button type="submit">
                        {editingPurpose ? "Update Multiplier" : "Create Multiplier"}
                      </Button>
                      <Button type="button" variant="outline" onClick={handleCancelPurpose}>
                        Cancel
                      </Button>
                    </div>
                  </form>
              ...
              {inspectionPurposes.map((purpose) => (
                <Card key={purpose.id}>
                  <CardHeader>
                    ...
                    {purpose.purposeEn && <CardDescription>{purpose.purposeEn}</CardDescription>}
                    <CardAction>
                      <div className="flex gap-2">
                        <Button
                          variant="outline"
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      <div>
                        <span className="text-sm text-gray-600">Multiplier: </span>
                        <span className="font-semibold">{purpose.multiplier}x</span>
                      </div>
                      <div>
                        <span className="text-sm text-gray-600">Status: </span>
                        <span className={purpose.isActive ? "text-green-600" : "text-red-600"}>
